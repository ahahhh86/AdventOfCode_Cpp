...
to a smaller sign that says "Danger - Personal Protective Equipment required beyond
this point".
...
into what must be the room that collects all of the light from outside. As you admire
the large assortment of lenses available to further focus the light, the reindeer
...
in the range 0 to 255. To run the HASH algorithm on a string, start with a current
...
offers the sum of the result of running the HASH algorithm on each step in the initialization
sequence.
...
of the results? (The initialization sequence is one long line; be careful when copy-pasting
it.)
...
are arranged in a line starting from the point where light enters the facility. The
boxes have holes that allow light to pass from one box to the next all the way down
the line.
...
to focus light passing through the box. The side of each box has a panel that opens
to allow you to insert or remove lenses as necessary.
...
The label will be immediately followed by a character that indicates the operation
to perform: either an equals sign (=) or a dash (-).
...
space made by removing the indicated lens. (If no lens in that box has the given
...
the box immediately behind any lenses already in the box. Don't move any of the other
...
All 256 boxes are always present; only the boxes that contain any lenses are shown
here. Within each box, lenses are listed from front to back; each lens is shown as
its label and focal length in square brackets.

To confirm that all of the lenses are installed correctly, add up the focusing power
of all of the lenses. The focusing power of a single lens is the result of multiplying
together:
...
export module Puzzle2023:D15;

namespace AOC::Y2023::D15 {
